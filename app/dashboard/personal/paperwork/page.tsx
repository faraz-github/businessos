...
export default function PersonalPaperworkPage() {
  ...
  useEffect(() => {
    async function fetch() {
      ...
      const { data: cl } = await supabase.from('clients').select('*').eq('user_id', user.id).eq('mode', mode);
    ...
    <PageTransition>
      <div className="space-y-6">
        ...
        <Tabs
          tabs={[
            { value: 'all', label: 'All' },
            ...docTypes.map((t) => ({ value: t.value, label: t.label })),
          ]}
          value={activeType}
            ...
            {documents.map((doc) => {
              ...
              return (
                <Card key={doc.id} variant="base" className="flex items-center gap-4">
                  <div className="w-9 h-9 rounded-[var(--radius-md)] bg-[var(--accent-blue-dim)] flex items-center justify-center shrink-0">
                    <FileText size={16} className="text-[var(--accent-blue)]" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-[13px] font-medium text-[var(--text-primary)] truncate">{doc.title || 'Untitled'}</p>
                      <Badge variant="outline" className="text-[9px] capitalize">{doc.type}</Badge>
...
function CreateDocumentModal({ open, onClose, mode, clients, onCreated }: {
  open: boolean; onClose: () => void; mode: string; clients: Client[];
