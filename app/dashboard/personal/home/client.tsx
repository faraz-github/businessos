...
function stagger(index: number) {
  return {
    ...
    transition: { delay: index * 0.06, duration: 0.32, ease: [0, 0, 0.2, 1] as any },
  };
}
...
export function PersonalHomeClient({ attentionItems, stats, priorities: initialPriorities, timeBlocks: initialTimeBlocks, recentLogs: initialLogs }: {
  ...
  async function handleTogglePriority(id: string, completed: boolean) {
    ...
    setPriorities(p => p.map(pr => pr.id === id ? { ...pr, completed: next } : pr));
  ...
  async function handleDeletePriority(id: string) {
    const prev = priorities;
    setPriorities(p => p.filter(pr => pr.id !== id));
  ...
  async function handleAddTimeBlock(type: TimeBlockType, startTime: string, endTime: string, label: string) {
    ...
    setTimeBlocks(prev => [...prev, res.data].sort((a, b) => a.start_time.localeCompare(b.start_time)));
  }
  ...
  // Precise current-block detection using minutes
  ...
  return (
    <PageTransition>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
        ...
        {/* Header */}
        <div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 32, fontWeight: 800, letterSpacing: '-0.8px', lineHeight: 1.05, color: 'var(--text-primary)' }}>
            Good {greeting}
          </h1>
          <p className="t-xs text-secondary" style={{ marginTop: 6 }}>Here&apos;s what needs your attention today.</p>
        </div>
            ...
            <div>
              ...
              {attentionItems.length === 0 ? (
                <div className="card" style={{ padding: '28px 24px', textAlign: 'center' }}>
                  ...
                  <p className="t-xs text-secondary">Nothing needs your attention right now. Nice work.</p>
                </div>
              ) : (
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <MetricCard icon={<Users size={15} />} iconColor="var(--accent-blue)"
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border-subtle)' }}>
                    <SubMetric label="Pipeline leads" value={String(stats?.clients?.pipelineLeads || 0)} />
                    <SubMetric label="All time" value={String(stats?.clients?.totalAllTime || 0)} />
                  </div>
                </MetricCard>

                {/* Outreach */}
                <MetricCard icon={<Linkedin size={15} />} iconColor="var(--accent-violet)"
                ...
                <MetricCard icon={<Briefcase size={15} />} iconColor="var(--accent-amber)"
                  label="Projects" value={stats?.work?.activeProjects || 0} sub="Active projects" />
              </div>
            </div>
          </div>
            ...
            <div className="card" style={{ padding: '16px 18px' }}>
              ...
              {recentLogs.length === 0 ? (
                <p className="t-xs text-tertiary" style={{ fontStyle: 'italic' }}>
                  Nothing logged yet. Hit Quick Log to capture something fast.
                </p>
              ) : (
...
function AddTimeBlockModal({ open, onClose, onAdd }: {
  ...
  function handleAdd() {
    onAdd(type, startTime, endTime, label);
    onClose();
  }
